...
const GoalTracker = ({ onUpdate }) => {
  ...
  const handleProgress = async (id, progress) => {
    try {
      ...
      // Update local state temporarily for snappy feeling
  ...
  // Triggers when user releases the slider handle
        ...
        <AnimatePresence>
          {showAdd && (
            <motion.form
              ...
              onSubmit={handleAdd}
              className="mb-5 space-y-2.5 overflow-hidden"
            >
              <input
                type="text"
              ...
              <div className="flex gap-2">
                ...
                <input
                  ...
                  className="zen-input w-[140px]"
                  id="goal-deadline-input"
                />
              </div>
              <button type="submit" className="zen-btn zen-btn-primary zen-btn-sm" id="goal-submit-btn">
                Establish Intention
              </button>
            </motion.form>
          )}
            ...
            <AnimatePresence>
              {goals.map((goal) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, x: -15 }}
                  className={`group bg-white/45 backdrop-blur-md rounded-xl p-4 border border-white/50 hover:border-white/80 transition-all ${
                ...
                >
                  <div className="flex items-start gap-3.5">
                    ...
                    <button
                      ...
                      className={`mt-0.5 w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all cursor-pointer ${
                        goal.completed
                          ...
                          : 'border-zen-400 hover:border-accent hover:bg-accent/5'
                      }`}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-semibold font-sans text-zen-800 ${goal.completed ? 'line-through text-zen-400' : ''}`}>
                        {goal.title}
                      </p>
                      {goal.notes && (
                        <p className="text-[11px] text-zen-500 mt-0.5 leading-relaxed">{goal.notes}</p>
                      )}
                      ...
                      {!goal.completed && (
                        <div className="flex items-center gap-3 mt-3">
                          ...
                          <input
                            ...
                            className="zen-slider w-20 h-1 cursor-pointer transition-opacity opacity-45 group-hover:opacity-100"
                            id={`goal-progress-${goal._id}`}
                            title="Drag to change progress"
                          />
                          <span className="text-[10px] text-zen-500 font-bold w-8 text-right font-sans">
                            {goal.progress}%
                          </span>
                        </div>
                      )}
                      ...
                      {goal.deadline && (
                        <div className="flex items-center gap-1 mt-2">
                          <span className="text-[9px] font-semibold text-accent uppercase tracking-widest font-sans">
